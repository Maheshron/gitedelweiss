				...
				if($pageURl[3]==$cpage)
		         {	
		         	if($_SESSION['admin_type'] == 'admin')
		         	{
						 ...
						 $pageName = 'http://'.$_SERVER['HTTP_HOST'].'/newedeweles/admin/view_journals.php';
						header("Location:".$pageName);
		         	}
		         	else if($_SESSION['admin_type'] == 'employee')
		         	{
						$pageName = 'http://'.$_SERVER['HTTP_HOST'].'/newedeweles/admin/about_journal.php?journal_id='.$journal_id;
						header("Location:".$pageName);
		         	}
		         }
